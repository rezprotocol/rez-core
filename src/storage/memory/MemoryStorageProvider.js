...
import { MemoryMailboxStore } from "./MemoryMailboxStore.js";
import { MemoryKeyValueStore } from "./MemoryKeyValueStore.js";
...
function normalizeListOptions(options) {
  ...
  if (typeof options.cursor === "string" && options.cursor) {
    normalized.cursor = options.cursor;
  }
  return normalized;
}
...
function createPeerLinkStorage(keyValueStore) {
  if (!keyValueStore) {
    throw new Error("createPeerLinkStorage requires keyValueStore");
  }
  return {
...
export class MemoryStorageProvider extends StorageProvider {
  constructor() {
    ...
    this.mailboxStore = new MemoryMailboxStore();
    this.keyValueStore = new MemoryKeyValueStore();
    this.peerLinkStorage = createPeerLinkStorage(this.keyValueStore);
  }
