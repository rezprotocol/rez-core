...
| Session persistence | At this audit baseline, `SecureSessionRecord`, `RatchetState`, and the then-existing node serializer persisted keys and counters without a suite ID ([session](../src/objects/ratchet/SecureSessionRecord.js#L16-L51), [state](../src/objects/ratchet/RatchetState.js#L20-L93)). DT-302 subsequently retired that dormant node persistence API; see the [current persistence model](../../rez-node/docs/PERSISTENCE.md). | A restored session cannot resolve a suite explicitly; retiring the duplicate store does not implement suite negotiation. |
